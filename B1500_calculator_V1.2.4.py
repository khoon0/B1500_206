...
class CSVFileProcessor(QMainWindow):
    ...
    def maybe_show_patch_notes(self):
        ...
        if last_seen != CURRENT_VERSION:
            # RichText 로 처리할 수 있게 <html> 태그 감싸고, <br><br> 로 두 줄 간격 확보
            notes = (
                "<html><body>"
                f"<b>새 버전 V{CURRENT_VERSION} 릴리스 노트</b><br><br>"
                "- 아이콘 뒤@지게 간지나는 아이언맨 느낌으로 바꿈<br><br>"
                "- Pr 값 정확도 아주 높게 추출되도록 쌈@뽕하게 수정함<br><br>"
                "- 그래프 축 값들 유효 숫자 야무지게 변경"
                "</body></html>"
            )
    ...
    def get_latest_version(self):
        try:
            ...
            return None  # 인터넷 연결이 없을 경우 None 반환
    ...
    def prompt_update(self, latest_version):
        ...
        self.start_update_process()  # '확인' 버튼 클릭 시 업데이트 프로세스 시작
            ...
            if filename.endswith(".exe"):
                ...
                if match:
                    ...
                    if version_tuple < current_version_tuple:
                        ...
                        try:
                            os.remove(file_path)
                            deleted_files.append(file_path)  # 삭제된 파일 경로 추가
                        except Exception as e:
                            self.show_message("파일 삭제 오류", f"파일 삭제 중 오류 발생: {e}")

        # 삭제된 파일 목록을 메시지 박스로 표시
        if deleted_files:
    ...
    def download_update(self):
        ...
        self.update_thread = UpdateThread(self)  # CSVFileProcessor 인스턴스를 전달
    ...
    def update_progress(self, value):
        if value == -1:
            self.show_message(f"Error downloading update", "다운로드 중 오류가 발생했습니다 {e}.")
    ...
    def get_user_inputs(self):
        while True:
            ...
            self.data_order_combo.addItems([
                "'Time', 'Voltage', 'Current'", "'Time', 'Current', 'Voltage'"
            ])
    ...
    def process_files(self, file_paths, file_names, measurement_method=None, dimensions=None):
        ...
        # 사용자에게 입력 받기 (입력이 없으면 get_user_inputs 호출)
    ...
    def msec_formatter(self, x, pos):
        s = f"{x:.2f}"
        if s.endswith("00"):      # 1.00 → "1"
            return str(int(round(x)))
        if s.endswith("0"):       # 1.50 → "1.5"
            return s[:-1]
        return s         
